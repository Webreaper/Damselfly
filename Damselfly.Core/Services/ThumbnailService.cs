...
public class ThumbnailService : IProcessJobFactory, IRescanProvider
{
    ...
    private bool BackgroundThumbnailProcessingEnabled =>
        _configService.GetBool( ConfigSettings.EnableBackgroundThumbs, false );
    ...
    private void DeleteThumbnails( IEnumerable<FileInfo> imagePaths )
    {
        ...
        foreach( var thumb in thumbPaths ) thumb.SafeDelete();
    }
    ...
    public async Task<FileInfo> GenerateFaceThumb(ImageObject face)
    {
        ...
        try
        {
            ...
            if( ! thumbPath.Exists )
            {
                ...
                var result = await ConvertFile( image, false, ThumbSize.Large);

                if( result.ThumbsGenerated == false )
                {
                    Logging.LogWarning(
                        $"Unable to generate thumb for {image.FullPath} - so skipping face thumb generation.");
                    return destFile;
                }
            }
